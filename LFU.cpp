    ...
    for (int i = 0; i < pages.size(); i++)
    {
        ...
        else
        {
            if (tmp.find(val) == tmp.end()) // not found
            {
                ...
                vector<int> tm;
                for (auto x : freq)
                { // for minimum frequency finding ...
                    if (x.second < mn and x.second > 0)
                    { // frequency 0 theke boro hoite hobe
                        mn = x.second;
                    }
                }
                for (auto x : freq)
                { // store minimum frequency values to the vector
                    if (mn == x.second)
                    {
                ...
                { // ektai minimum value pele oitai
                    str = tm[0];
                    for (int kk = 0; kk < i; kk++)
                    {
                        if (str == pages[kk])
...
int main()
{
    //    vector<int>a= {7,0,1,2,0,3,0,4,2,3,0,3,2,1,2}; //3->10
    vector<int> a = {7, 0, 2, 4, 3, 1, 4, 7, 32, 0, 4, 3, 0, 3, 2, 7};
    // vector<int>a ={ 5, 0, 1, 3, 2, 4, 1, 0, 5} ;//4->8
    int siz;
    cin >> siz;
    LFU(a, siz);
}
